...
def scan_oracle_database(host, port, database, user, password):
    import psycopg2
    console.print(f"\n[bold magenta]Meridian — Oracle PostgreSQL Database Scanner[/bold magenta]")
    ...
    try:
        ...
        console.print("[yellow]Tip: Check host, port, credentials and SSL settings[/yellow]")
        raise
    except Exception as e:
...
def print_oracle_db_summary(result):
    console.print("\n" + "─" * 50)
    console.print("[bold magenta]  Meridian — Oracle PostgreSQL Database Scan[/bold magenta]")
    console.print("─" * 50)
    ...
    if result['tables']:
        ...
        console.print("  [yellow]No tables found — empty database[/yellow]")
